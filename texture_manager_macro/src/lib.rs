...
#[derive(Debug)]
struct FolderNode {
    textures: Vec<(String, String)>, // (field_name, file_path) - single textures
    texture_arrays: Vec<(String, Vec<String>)>, // (field_name, vec of file_paths) - frames/variants
...
fn parse_frame_or_variant(file_stem: &str) -> Option<(String, usize)> {
    // Returns: (basename, frame_number)
    // Example: "walk_f0" -> Some(("walk", 0))
    // Example: "idle_v2" -> Some(("idle", 2))
...
fn build_folder_tree(assets_path: &Path, assets_folder_str: &str) -> FolderNode {
    ...
    // First pass: collect all files organized by their parent directory
                ...
                if let Some(extension) = path.extension() {
                    ...
                    if ext == "png" || ext == "jpg" || ext == "jpeg" || ext == "bmp" || ext == "gif" {
                        if let Some(file_stem) = path.file_stem() {
                            ...
                            // Get the relative path from assets root
    ...
    // Second pass: process files by directory and group frames/variants
    for (_dir_path, files) in files_by_dir {
        // Group files by basename if they're frames or variants
        ...
        // Process grouped textures (frames/variants)
        for (basename, indexed_files) in grouped {
            // Convert BTreeMap to sorted Vec by index
            ...
            if !sorted_files.is_empty() {
                ...
                // Navigate to the correct folder node
...
fn generate_struct_for_folder(
    ...
    // Generate struct name based on full path to avoid conflicts
    ...
    // Sort textures for deterministic output
